...
const PopularProduct = ({ name, id, imgURL, price, className }) => {
  ...
  return (
    <div className={"flex flex-col justify-center bg-gray-100 w-full rounded-xl p-5" + customClass}>
      <div className="flex flex-col items-center">
        <img src={imgURL} alt={name} className="w-20 md:w-40 rounded-lg aspect-square mb-5"/>
        <div className="text-center">
          <p className="text-regular md:text-lg font-semibold break-words">{name}</p>
          <p className="text-regular md:text-lg font-regular italic text-gray-600 pb-5">${price}</p>
        </div>
      </div>
